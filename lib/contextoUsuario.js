...
/* Data SEMPRE no horario de Brasilia (23/09/2026). O servidor roda em UTC:
   com getDate() puro, tudo o que acontecia entre 21h e meia-noite saia com o
   dia seguinte — o story publicado as 23h26 de 22/09 aparecia como 23/09, e o
   mesmo valia para liberacao de comissao, saque e conclusao das aulas.
   Brasil sem horario de verao desde 2019: -3 h fixo e o plano B se o Intl
   do ambiente nao tiver o fuso. */
function dataBrMs(m) {
  if (!m) return '';
  try {
    return new Date(m).toLocaleDateString('pt-BR', { timeZone: 'America/Sao_Paulo', day: '2-digit', month: '2-digit', year: 'numeric' });
  } catch (_) {
    const d = new Date(m - 3 * 3600000);
...
async function montarContexto(uid, email, opcoes) {
  ...
  /* 23/09 (rodada 3): ativo E dentro do vencimento — o teste vencido ficava
     com ativo:true e o Vik dizia "Pro ATIVO" para quem ja caiu no Basico. */
  const venceMsA = assinatura ? ms(assinatura.vence_em) : 0;
  const planoAtivo = !!(assinatura && assinatura.ativo === true && (!venceMsA || venceMsA > Date.now()));
  const emTesteGratis = planoAtivo && (assinatura.periodo === 'trial' || ms(assinatura.testeAte) > Date.now());
  ...
  if (assinatura) {
    ...
    if (emTesteGratis) {
      if (assinatura.periodo === 'trial') linhas.push('Esta no TESTE GRATIS ate ' + dataBr(assinatura.vence_em) + ' — pode assinar ja em Meu Plano: a 1a cobranca so vence no fim do teste e nao perde nenhum dia.');
      else linhas.push('JA ASSINOU durante o teste gratis: o plano pago conta a partir de ' + dataBr(assinatura.testeAte) + ', e ate la segue com tudo o que o teste da (fotos, videos e live do teste).');
      linhas.push('Periodo de teste NAO gera comissao para quem indicou. Se esta conta tambem for PARCEIRA, as indicacoes dela nao geram comissao enquanto o proprio plano nao estiver pago (regulamento 2.4).');
    }
  } else {
  ...
  if (negocio) {
    ...
    // 23/09 (rodada 3): o teste gratis tambem tem live (nivel Premium, com a cota do teste) — o servidor ja tratava assim.
    const planoLive = planoAtivo && (['premium', 'enterprise'].indexOf(planoId) >= 0 || emTesteGratis);
    if (podeLive === null) {
  ...
  if (parceiro) {
    ...
    if (parceiro.criador === true) {
      ...
      if (pc) {
        ...
        linhas.push('Liberadas para publicacao (aprovada E autorizada): ' + pc.liberadas +
          '. Dia e hora sao escolhidos pelo sistema de publicacao do Moviki — nao existe data marcada.');
    ...
    // Clausula 2.4 (23/09/2026): parceiro que tambem e lojista so gera
    // comissao com o plano do proprio negocio pago e em dia. Mesma conta do
    // painel (parceiro.html, avisoLojistaSemPlano): ativo, nao vencido e
    // periodo diferente de 'trial'. Basico conta como nao pago.
    if (negocio) {
      const venceMs = ms(assinatura && assinatura.vence_em);
      const pagante = !!(assinatura && assinatura.ativo === true && (!venceMs || venceMs > Date.now()) &&
        assinatura.periodo !== 'trial' && String(assinatura.plano || 'basico').toLowerCase() !== 'basico');
      linhas.push(pagante
        ? 'Clausula 2.4 (parceiro que tambem e lojista): cumprida — o plano do proprio negocio esta pago e em dia, as indicacoes geram comissao.'
        : 'Clausula 2.4 (parceiro que tambem e lojista): NAO cumprida — o plano do proprio negocio nao esta pago e em dia (teste gratis, Basico ou vencido). Enquanto isso, os pagamentos dos indicados NAO geram comissao, nem depois. O painel mostra o aviso amarelo no topo da Visao geral.');
    }
